...
export class ProductoService {
  ...
  getProductsAll(pull: boolean = false) {
    ...
    return this.http.get<RespuestaProducto>(`${URL}/productos/inicio/?pagina=${this.paginaSales}`);
  ...
  getMySale(id: string): Observable<any> {
    ...
    return this.http.get(`${URL}/productos/mycom/` + id, { headers });
  }
  ...
  updateOfferNoPhoto(offer: any): Observable<Offer> {
    ...
    return this.http.put(`${URL}/productos/updated/${offer._id}`, params, { headers });
  ...
  updateOffer(offer: any, photo: File): Observable<Offer> {
    ...
    return this.http.put(`${URL}/productos/updateimg/${offer._id}`, fd, { headers });
  ...
  createCom(sale: any, photo: File): any {
    ...
    return new Promise(resolve => {
      ...
      // Guardar rubro como array en formdata
      const arr = sale.rubro;
      for (let i = 0; i < arr.length; i++) {
        fd.append('rubro', arr[i]);
      }

      const bs = sale.bussines_days;
      for (let b = 0; b < bs.length; b++) {
        fd.append('bussines_days', bs[b]);
      }

      fd.append('coords', sale.coords);
      fd.append('commerce', sale.commerce);
      fd.append('linkinsta', sale.linkinsta);
      fd.append('linkface', sale.linkface);
      fd.append('linkweb', sale.linkweb);

      fd.append('street', sale.street);
      fd.append('province', sale.province);
      fd.append('municipality', sale.municipality);
      fd.append('location', sale.location);
      fd.append('neighborhood', sale.neighborhood);
      fd.append('bio', sale.bio);
      fd.append('cellphone', sale.cellphone);
      fd.append('telephone', sale.telephone);
      fd.append('whatsapp', sale.whatsapp);
      fd.append('delivery', sale.delivery);
      fd.append('card', sale.card);
      fd.append('opening_hours', sale.opening_hours);
      fd.append('closing_schedule', sale.closing_schedule);
      fd.append('tags', sale.tags);
